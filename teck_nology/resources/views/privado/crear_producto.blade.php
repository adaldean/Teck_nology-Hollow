...
<body>
        ...
        <main class="main-content">
            ...
            <section class="seccion_formulario">
                <div class="card" style="max-width:950px;margin:20px auto;padding:20px;background:#fff;border-radius:8px;">
                    @php
                        // Si accedemos desde la ruta de desarrollo (/dev/...), enviamos el formulario al endpoint de prueba
                        $devAction = request()->is('dev/*') ? url('/dev/inventario/guardar') : route('inventario.store');
                    @endphp

                    <form action="{{ $devAction }}" method="POST" enctype="multipart/form-data" class="form-grid">
                        @csrf
                        <div class="form-row">
                        ...
                        <div class="form-row">
                            ...
                            <select name="id_proveedor" id="id_proveedor">
                                ...
                                @foreach($proveedores as $prov)
                                    <option value="{{ $prov->id_proveedor }}" {{ old('id_proveedor') == $prov->id_proveedor ? 'selected' : '' }}>{{ $prov->nombre }}</option>
                                @endforeach
                            </select>
                        </div>
    ...
    <script>
        // Pequeños estilos en línea para el layout del formulario si la hoja no cubre todo
        (function(){
